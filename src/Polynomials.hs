...
import Data.List.Toolbox (dropWhileEnd, genericLength, genericReplicate, genericTake, stripPrefix)
...
zipPoly f x@(Poly xs) y@(Poly ys) = poly $ zipWith f xs' ys'
  where
    d = deg x - deg y
